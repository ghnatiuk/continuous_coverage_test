...
static int
coverage_continuous_result_i(st_data_t key, st_data_t val, st_data_t h)
{
 ...
 */
static VALUE
rb_continued_coverage_result(VALUE klass)
{
    ...
    st_foreach(RHASH_TBL(coverages), coverage_continuous_result_i, ncoverages);
    return ncoverages;
}
 ...
 * = Usage
 *
 * 1. require "ccoverage.so"
 * 2. do Coverage.start
 * 3. require or load Ruby source file
 * 4. Coverage.continuous_result will return a hash that contains filename as key and
...
Init_ccoverage(void)
{
    VALUE rb_mCoverage = rb_define_module("CCoverage");
    rb_define_module_function(rb_mCoverage, "start", rb_coverage_start, 0);
    rb_define_module_function(rb_mCoverage, "continuous_result", rb_continued_coverage_result, 0);
